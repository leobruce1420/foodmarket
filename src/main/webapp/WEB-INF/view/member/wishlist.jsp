...
<div class="col-md-9 ml-sm-auto col-lg-10 px-md-4 pt-3 pb-2 mb-3 border-bottom">
 <div class="container">
 	...
 	<table id="contentTable" class="table table-condensed ">
		<tr>
		<th style="width:20px;"><th style="width:150px;"><th style="width:150px;"><th style="width:150px;"><th style="width:80px;">
		<c:forEach var="wishList" items="${page.content}">
		
			<tr id="${wishList.product.getProductid()}" class="text-center"><td >${wishList.product.getProductid()}</td>
			...
			<td ><button type="button" class="btn btn-danger" id="delete${wishList.product.getProductid()}" onclick="checkDelete(${wishList.product.getProductid()},${loginUserId})">取消收藏</button></td>
	...
	<nav aria-label="Page navigation example">
  		<ul class="pagination">
  		 ...
  		 <c:choose>
			...
			<c:otherwise>
			<li class="page-item">
			 	<a class="page-link" href="${contextRoot}/wishList/${loginUserId}?p=${page.number+1 -1}" aria-label="Previous">
        		<span aria-hidden="true">&laquo;</span>
      			</a>
      		</li>
			</c:otherwise>		
  		...
  		<c:forEach var="pageNumber" begin="1" end="${page.totalPages}">
   		<c:choose>
			...
			<c:when test="${pageNumber == page.number+1}">
			 <li class="page-item disabled"><a class="page-link" href="${contextRoot}/wishList/${loginUserId}?p=${pageNumber}"><c:out value="${pageNumber}" /></a></li>
			</c:when>
			
			<c:otherwise>
			<li class="page-item"><a class="page-link" href="${contextRoot}/wishList/${loginUserId}?p=${pageNumber}"><c:out value="${pageNumber}" /></a></li>
			</c:otherwise>
   		...
   		<c:choose>
			...
			<c:otherwise>
			<li class="page-item">
     		 	<a class="page-link" href="${contextRoot}/wishList/${loginUserId}?p=${page.number +2}" aria-label="Next">
        	 	<span aria-hidden="true">&raquo;</span>
      		 	</a>
    		</li>
			</c:otherwise>		
